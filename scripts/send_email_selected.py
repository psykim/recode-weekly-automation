#!/usr/bin/env python3
"""
선택한 보고서를 이메일로 발송하는 스크립트
특정 보고서 파일을 지정하여 수신자들에게 이메일로 발송합니다.
"""
...
class EmailSender:
    ...
    def create_email_message(self, recipient: dict, report_path: str) -> MIMEMultipart:
        ...
        if 'recode_weekly_' in report_filename:
            date_part = report_filename.split('_')[2]  # 20250721
            year = date_part[0:4]
            month = date_part[4:6]
            day = date_part[6:8]
        ...
        msg['Subject'] = f"RECODE WEEKLY - {report_date} 주간 논문 리뷰"
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>{report_date} RECODE WEEKLY가 도착했습니다.</p>
                    
                    <p>최신 치매 연구 동향과 주요 논문들을 한눈에 확인하실 수 있습니다.</p>
                    
                    <center>
                        <a href="https://psykim.github.io/recode-weekly-automation/reports/{report_filename}" class="button">
                            온라인에서 보기
                        </a>
                    </center>
        ...
        text_body = f"""
RECODE WEEKLY - {report_date}

안녕하세요, {recipient.get('name', '구독자')}님

{report_date} RECODE WEEKLY가 도착했습니다.

최신 치매 연구 동향과 주요 논문들을 확인하시려면 
첨부된 HTML 파일을 열어보시거나 아래 링크를 방문해주세요:

https://psykim.github.io/recode-weekly-automation/reports/{report_filename}
...
"""
    ...
    def send_to_all(self, report_path: str):
        ...
        print(f"선택한 보고서: {os.path.basename(report_path)}")
...
def main():
    # 환경 변수에서 보고서 파일명 확인 (GitHub Actions 용)
    report_filename = os.environ.get('REPORT_FILENAME')
    
    # 환경 변수가 없으면 명령줄 인자 확인
    if not report_filename and len(sys.argv) >= 2:
        report_filename = sys.argv[1]
    
    if not report_filename:
        print("보고서 파일명을 지정해주세요.")
        print("사용법: python send_email_selected.py <report_filename>")
        print("또는 REPORT_FILENAME 환경 변수를 설정하세요.")
        sys.exit(1)
    ...
    if not os.path.exists(report_path):
        print(f"보고서 파일을 찾을 수 없습니다: {report_filename}")
        sys.exit(1)
